export const IMAGENES = [
  {
    id: 0,
    titulo: "Águila",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "aguila.jpg",
    etiquetas: ["animal", "ave", "naturaleza"],
    comentarios: [{
      usuario: "Alan Brito",
      comentario: "Con su mirada otea el horizonte."
    }]
  },
  {
    id: 1,
    titulo: "Barca en la orilla",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "barca.jpg",
    etiquetas: ["barca", "rio", "paisaje", "naturaleza", "bosque"],
    comentarios: []
  },
  {
    id: 2,
    titulo: "Chica sobre lavanda",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "chicalavanda.jpg",
    etiquetas: ["mujer", "chica", "naturaleza", "lavanda"],
    comentarios: [{
      usuario: "Elena Nito",
      comentario: "Qué bien huele la la lavanda después de un día de lluvia."
    }]
  },
  {
    id: 3,
    titulo: "Desayuno",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "desayuno.jpg",
    etiquetas: ["comida", "desayuno", "huevos", "pan", "flores", "fruta"],
    comentarios: [{
      usuario: "Elba Lazo",
      comentario: "No hay nada como empezar el día con un desayuno de campeones."
    },
    {
      usuario: "Elsa Pato",
      comentario: "Los huevos aportan proteinas al desayuno. Todas las mañanas me como un par de huevos fritos."
    }]
  },
  {
    id: 4,
    titulo: "Postre",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "dessert.jpg",
    etiquetas: ["comida", "postre", "dulce"],
    comentarios: []
  },
  {
    id: 5,
    titulo: "Fresas",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "fresas.jpg",
    etiquetas: ["comida", "fruta", "fresa"],
    comentarios: []
    
  },
  {
    id: 6,
    titulo: "Gato negro",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "gatonegro.jpg",
    etiquetas: ["gato", "animal", "mascota", "negro"],
    comentarios: []
  },
  {
    id: 7,
    titulo: "Uva",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "grapes.jpg",
    etiquetas: ["fruta", "comida", "verde"],
    comentarios: [{
      usuario: "Leandro Gado",
      comentario: "Uvas con queso saben a beso."
    },
    {
      usuario: "Esteban Quito",
      comentario: "La uva tiene muchos antioxidantes."
    },
    {
      usuario: "Elsa Pato",
      comentario: "La uva es una fruta obtenida de la vid. Las uvas vienen en racimos, son pequeñas y dulces. Se comen frescas o se utilizan para producir agraz, mosto, vino, vinagre y pisco. Crecen agrupadas en racimos de entre 6 y 300 uvas. Pueden ser negras, moradas, amarillas, doradas, púrpura, rosadas, marrones, anaranjadas o blancas, aunque estas últimas son realmente verdes y evolutivamente proceden de las uvas rojas con la mutación de dos genes que hace que no desarrollen antocianinas, siendo estas las que dan la pigmentación."
    }]
  },
  {
    id: 8,
    titulo: "Castillo en la niebla",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "niebla.jpg",
    etiquetas: ["paisaje", "castillo", "lago", "niebla", "naturaleza"],
    comentarios: []
  },
  {
    id: 9,
    titulo: "Pareja en bicicleta",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "pareja.jpg",
    etiquetas: ["hombre", "mujer", "pareja", "bicicleta", "naturaleza"],
    comentarios: []
  },
  {
    id: 10,
    titulo: "Bicicleta verde con flores",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "biciverde.jpg",
    etiquetas: ["bicicleta", "verde", "flores"],
    comentarios: []
  },
  {
    id: 11,
    titulo: "Diente de león",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "dienteleon.jpg",
    etiquetas: ["planta", "blanco", "naturaleza"],
    comentarios: []
  },
  {
    id: 12,
    titulo: "Chica en moto",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "guzzi.jpg",
    etiquetas: ["chica", "mujer", "moto", "Guzzi", "naturaleza", "flores", "amapolas"],
    comentarios: []
  },
  {
    id: 13,
    titulo: "La Farola de Málaga",
    descripcion: "No puedor ahorarr ese hombree al ataquerl al ataquerl pupita torpedo. Por la gloria de mi madre se calle ustée hasta luego Lucas caballo blanco caballo negroorl a wan benemeritaar me cago en tus muelas. Te va a hasé pupitaa no te digo trigo por no llamarte Rodrigor diodeno quietooor llevame al sircoo papaar papaar.",
    fichero: "lafarola.jpg",
    etiquetas: ["Málaga", "puerto", "faro", "farola", "barcos", "mar", "paseo", "muelle"],
    comentarios: []
  }
];